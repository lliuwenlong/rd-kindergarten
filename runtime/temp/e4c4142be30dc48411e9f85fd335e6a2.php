<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:42:"./application/admin/view/teach/record.html";i:1548654015;s:55:"/home/www/yey/application/admin/view/public/layout.html";i:1548654013;}*/ ?>
<!doctype html>
<html>
<head>
	...
	<div class="fixed-bar">
		...
		<div class="flexigrid">
				<div class="tDiv">
		            <div class="tDiv2">
		                <div class="fbutton">
		                    <a href="<?php echo U('Teach/mu_add',['id'=>0]); ?>">
		                        <div class="add" title="前去采购">
		                            <span><i class="fa fa-plus"></i>学生档案——模板添加</span>
		                        </div>
		                    </a>
			...
			<tbody id="tbd">
				<?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): if( count($list)==0 ) : echo "" ;else: foreach($list as $key=>$val): ?>
						...
						<tr>
							...
							<td><img style="width:50px;height:50px;" src="/public/img/<?php echo $val['student_img']; ?>"  ></td>
							<td><a href="/index.php?m=Admin&c=Teach&a=recordXQ&id=<?php echo $val['id']; ?>"><button type="button">档案详情</button></a></td>
...
<script>
	...
	$(document).on('click',".cl",function(){
		...
		$.ajax({
			url:"/index.php?m=Admin&c=Teach&a=getrecordData",
			data:{"arr":arr},
			type:'post',  
			dataType:'json',
			success:function(res){
				...
				$.each(res,function(i,val){
				    str +=`
						...
						<tr>
							<td>${val.id}</td>
							<td>${val.name}</td>
							<td><img style="width:50px;height:50px;" src="/public/img/${val.student_img}"  ></td>
							<td><a href="/index.php?m=Admin&c=Teach&a=recordXQ&id=${val.id}"><button type="button">档案详情</button></a></td>
