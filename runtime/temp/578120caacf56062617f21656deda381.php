<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:42:"./application/admin/view/teach/patrol.html";i:1548654015;s:55:"/home/www/yey/application/admin/view/public/layout.html";i:1548654013;}*/ ?>
<!doctype html>
<html>
<head>
			...
			<tbody id="tbd">
				<?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): if( count($list)==0 ) : echo "" ;else: foreach($list as $key=>$val): ?>
					<tr>
						...
						<td class="img"><?php echo $val['img']; ?><img src="/public/img/<?php echo $val['img']; ?>" style="display: none;width:50px;height:50px;" alt=""></td>	
						<td><?php echo $val['name']; ?></td>	
						<td><?php echo $val['desc']; ?></td>	
						<td><?php echo $val['sug']; ?></td>	
						<td><?php echo $val['genjin']; ?></td>
						<td><?php echo $val['garden_id']; ?></td>
...
<script>
	...
	$(document).on('click',".cl",function(){
        ...
        $.ajax({
			url:"/index.php?m=Admin&c=Teach&a=getpatrolData",
			data:{"arr":arr},
			type:'post',  
			dataType:'json',
			success:function(res){
				...
				$.each(res,function(i,val){
				    str +=`
				    	<tr>
						<td>${val.id}</td>
						<td class="img">${val.img}<img src="/public/img/${val.img}" style="display: none;width:50px;height:50px;" alt=""></td>
						<td>${val.name}</td>
						<td>${val.desc}</td>
						<td>${val.sug}</td>
						<td>${val.genjin}</td>
